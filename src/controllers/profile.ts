...
export const updateProfile = (req: any, res: any) => {
    const { id } = req.user
    Profile.findOneAndUpdate({ userId: id }, req.body, {upsert: true}).then((result) => {
        res.status(200).json({
            code: 200,
...
export const uploadProfilePic = async (req: any, res: any) => {
    ...
    uploadImage(path, 'blue_bird_dps').then((data) => {
        Profile.findOneAndUpdate({ userId: req.user.id }, { image: data.url }, {upsert: true}).then(() => {
            res.status(200).json({
                code: 200,
                message: 'Uploaded',
                data
...
export const getProfileData = (req: any, res: any) => {
    Profile.findOne({ userId: req.user.id }).then((data) => {
        res.status(200).json({
            code: 200,
